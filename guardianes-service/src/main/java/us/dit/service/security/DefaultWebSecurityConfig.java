...
public class DefaultWebSecurityConfig {
	...
	SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		http.authorizeHttpRequests((authorizeHttpRequests) -> authorizeHttpRequests
				.antMatchers("/rest/*", "/procesohola/nuevo").authenticated().antMatchers("/*", "/img/*").permitAll())
	...
	UserDetailsService userDetailsService(BCryptPasswordEncoder encoder) {
		...
		UserDetails user = User.withUsername("user")
				.password(encoder.encode("user")).roles("kie-server").build();
	...
	/**
	 * Componente para obtener la password de usuario en claro
	 * La única implementación tiene las password en memoria, será necesario implementar una clase que las obtenga de un sistema de persistencia o de un servicio
